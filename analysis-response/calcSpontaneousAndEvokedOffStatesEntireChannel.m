function offState = calcSpontaneousAndEvokedOffStatesEntireChannel(raster, timeMs,params)
...
frPerTrialPostOnset = nan(nTrials,1);

for iTrial = 1:nTrials
    currentIsis = diff(find(raster(iTrial,isDuringEntireBaselineTime)));
    [pCurrentTrial,~] = gamfit(currentIsis);
    minIsiOffStateCurrentTrial = gaminv(params.pIsiToDefineOffState,pCurrentTrial(1),pCurrentTrial(2));
    ...
    if isiAroundMidPostOnset>=minIsiOffStateCurrentTrial
    ...
    if isiAroundMidBaseline>=minIsiOffStateCurrentTrial
        isOffStatePerTrialBaseline(iTrial) = true;
    end
    probInOffState(iTrial) = sum(currentIsis(currentIsis>minIsiOffStateCurrentTrial))./sum(currentIsis); 
end
