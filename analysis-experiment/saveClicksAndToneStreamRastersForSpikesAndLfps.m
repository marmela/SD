function saveClicksAndToneStreamRastersForSpikesAndLfps(allSessions,isClicks)
...
for iSess = nSessions:-1:1
    ...
    if isClicks
        ...
        stateFileName = sprintf('ToneStream-%s-%d',sessInfo.animal,sessInfo.animalSession);
    end
    ...
    epochFileName = getEpochsFileName(sessInfo);
    if ~isClicks
        epochFileName = [epochFileName, '_Tones'];
    end
    save([epochsDir filesep epochFileName], 'statePerStimValidTrials',...
    ...
    if ~isClicks
        eegFileName = [eegFileName, '_Tones'];
    end
    ...
    for ch = channels
        ...
        if ~isClicks
            epochsFileName = [epochsFileName, '_Tones'];
        end
